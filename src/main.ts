...
import { assessStairway } from './escalator';
...
// Step counts come from the cached measurements when the source index leaves them blank,
// so the assessment reasons about a real staircase instead of a rating-based guess.
const readings = new Map(stairs.map(stair => [stair.id, stepReading(stair.id, stair.steps)]));
const assessments = new Map(stairs.map(stair => {
  const reading = readings.get(stair.id);
  return [stair.id, assessStairway(reading && reading.source !== 'index'
    ? { ...stair, measuredSteps: reading.steps, measuredStepSource: reading.source }
    : stair)];
}));
...
el('#app').innerHTML = `
  ...
  <main>
    ...
    <section class="explorer" aria-label="Explore stairways">
      ...
      <div class="filter-bar"><span class="filter-label">EXPLORE BY RATING</span><div class="rating-filters"><button class="chip active" data-rating="all" aria-pressed="true">All stairs</button>${[5,4,3,2,1].map(r => `<button class="chip" data-rating="${r}" aria-pressed="false"><span class="dot" style="--dot:${colors[r]}"></span>${r}<span class="chip-extra"> · ${['','','Local favorites','Hidden gems','Impressive','Extraordinary'][r] || 'Everyday'}</span></button>`).join('')}</div><div class="filter-toggles"><label class="photo-filter" title="Filter results to stairways with visible photo previews"><input type="checkbox" id="photos-only"> Only show stairways with photo previews</label><label class="photo-filter retrofit-filter" title="Filter results to stairways flagged as escalator retrofit candidates"><input type="checkbox" id="retrofit-only"> Only show escalator retrofit candidates</label></div></div>
      <div class="explorer-body"><aside class="results-panel"><div class="results-heading"><div><h2>Your next discovery</h2><p id="result-count" aria-live="polite"></p></div><button id="reset" class="reset">Reset</button></div><div id="results" class="results"></div></aside><div class="map-wrap"><div id="map" aria-label="Interactive San Francisco stairway map"></div><div class="map-badge"><span class="live-dot"></span> THE CITY IS BETTER ON FOOT</div><div class="map-actions"><button id="locate" aria-label="Find my location" title="Find my location">${icon('locate')}</button><button id="fit" aria-label="Fit all filtered stairways" title="Fit filtered stairways">${icon('pin')}</button></div><details class="map-legend" open><summary>Map legend <span>⌃</span></summary><div>${[5,4,3,2,1,0].map(r=>`<div><span class="dot" style="--dot:${colors[r]}"></span><b>${r || '?'}</b> ${labels[r]}</div>`).join('')}<div class="legend-retrofit"><span class="retrofit-flag">Retrofit</span> Escalator retrofit candidates are flagged</div><button id="legend-button">What do the ratings mean? ↗</button></div></details><div id="detail" class="detail" hidden></div><p id="map-status" role="status" hidden></p></div></div>
    </section>
